...
func (r *Repository) UniverseEntries(ctx context.Context, provider, universe string) ([]UniverseEntry, error) {
	...
	// The newest stored session is what separates a symbol that still works from one that
	// quietly stopped; the bar count is reported alongside it but never judged on, because a
	// long history says nothing about whether it is still growing.
	rows, err := r.pool.Query(ctx, `SELECT i.ticker,i.isin,i.name,e.mic,p.provider_symbol,
			...
			coalesce((SELECT max(b.session_date)::text FROM daily_price_bars b
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entry.Ticker, &entry.ISIN, &entry.Name, &entry.MIC, &entry.ProviderSymbol, &entry.StoredBars, &entry.LastSession); err != nil {
