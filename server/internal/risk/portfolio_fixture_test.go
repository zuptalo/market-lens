...
func (f *portfolioFixture) portfolioService() *portfolio.Service {
...
func (f *portfolioFixture) record(user portfolio.UUID, ticker string, direction portfolio.Direction,
	...
	f.t.Helper()
	trade, err := f.portfolioService().Record(f.ctx, user.String(), portfolio.RecordRequest{
...
func (f *portfolioFixture) view(user portfolio.UUID) portfolio.View {
	f.t.Helper()
	view, err := f.portfolioService().View(f.ctx, user.String())
	if err != nil {
