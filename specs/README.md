...
| 024 | [An Overview that says what needs you](024-overview/spec.md) | Planned | Pending | Pending | 014, 016, 017, 022, 023 | Replaces a foundation-stage stub that had begun misstating the product's own state. Answers what needs a person and what changed, collecting decisions only a person can make from the three screens they were scattered across. Counts and dates only — no value, percentage or return, because every such figure would be a second copy of one another screen owns. Adds no migration, no endpoint and no event: it composes reads that already exist. |
