...
| 009 | [Self-provisioned signing key](009-self-provisioned-keys/spec.md) | In review | [Complete](009-self-provisioned-keys/plan.md) | [Complete](009-self-provisioned-keys/tasks.md) | 004 | Reduces required production configuration to `DATABASE_URL`. `AUTH_SECRET` becomes self-provisioned and database-resident; `EXTERNAL_CREDENTIAL_KEY` stays external because it encrypts credentials held in that same database. |
| 010 | [Actionable owner setup errors](010-setup-error-clarity/spec.md) | In review | In spec | In spec | 004 | Setup names every field the operator must fix instead of answering "The request is invalid.", and verifies SMTP before storing it so broken mail is caught at setup rather than at the first invitation. |
| 011 | [Owner integration settings](011-integration-settings/spec.md) | In review | In spec | In spec | 004, 010 | The owner can see, check, and change the EODHD and SMTP configuration. Nothing is stored until it verifies, and secrets stay write-only. Replaces a write-once configuration whose only recovery was a new database. |
| 012 | [One component library everywhere](012-primevue-consistency/spec.md) | In review | In spec | In spec | 004, 010, 011 | Every screen uses PrimeVue rather than hand-rolled controls, enforced by a test. Fixed three latent accessibility defects in the theme and one lost heading structure. |
