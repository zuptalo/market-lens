...
- [ ] T003 [P] Generalize `server/internal/api/contract_test.go` so `contractPath` becomes a list of contract files rather than the single hard-coded `specs/004-owner-access/contracts/openapi.yaml`, keeping the suite green with only the 004 contract registered
...
- [ ] T033 [US2] Register `specs/005-instrument-exploration/contracts/openapi.yaml` in the contract list added by T003 in `server/internal/api/contract_test.go`, so both route reconciliation and the `x-market-lens-access-boundary` assertion cover this feature. Run it and record the red: `/instruments/{id}/history` is documented but not routed
...
- [ ] T077 [P] Assert the listing performance bound in `server/internal/instruments/repository_integration_test.go`: the first page of the full fixture universe returns within the SC-002 budget under every supported sort key
