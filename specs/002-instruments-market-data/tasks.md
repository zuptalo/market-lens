...
- [ ] T010 [US1] Add the required baseline-upgrade integration test for two equal tickers on different MICs plus identity/foreign-key constraints in `server/internal/instruments/repository_integration_test.go`; run against migration `0001` and record the expected missing-domain red result
- [ ] T011 [P] [US1] Add repository/service tests for idempotent synchronization, allowed metadata updates, stable UUID preservation, provider mappings, inactive retention, conflicting identity rejection, and an immutable sanitized `universe_sync` run record in `server/internal/instruments/service_test.go`; run and record the expected red behavior
- [ ] T012 [P] [US1] Add a migration seed audit proving exactly 100 valid common-equity memberships, 25 per selected Nordic primary exchange, with traceable curation metadata in `server/internal/db/nordic_universe_test.go`; run and record the expected red behavior

### Implementation for User Story 1

- [ ] T013 [US1] Add ordered exchange, instrument, provider-instrument, research-universe, and membership schema with search/identity constraints in `server/internal/db/migrations/0002_instruments.sql`
- [ ] T014 [US1] Implement explicit-SQL instrument persistence, search primitives, and transactional synchronization in `server/internal/instruments/repository.go`
- [ ] T015 [US1] Implement provider-neutral universe synchronization, conflict/inactivation rules, and immutable sanitized run accounting in `server/internal/instruments/service.go`
- [ ] T016 [US1] Add the reviewed four-exchange, exactly-100-listing reference universe as a forward migration with selection provenance in `server/internal/db/migrations/0003_nordic_universe.sql`
- [ ] T017 [US1] Run the clean-database and `0001` upgrade paths plus the US1 suites, and record green identity, synchronization, and seed evidence in `specs/002-instruments-market-data/quickstart.md`
...
- [ ] T018 [P] [US2] Add migration upgrade/constraint tests for sessions, bars, revisions, actions, import runs/items, findings, daily uniqueness, numeric ranges, and referential integrity in `server/internal/db/market_data_migrations_test.go`; run and record the expected red behavior
- [ ] T019 [P] [US2] Add a calendar migration audit for official-source traceability, the ten-year window plus next complete year, time-zone correctness, closure/non-session handling, and all four MICs in `server/internal/db/nordic_calendars_test.go`; run and record the expected red behavior
- [ ] T020 [P] [US2] Add provider contract tests for instrument resolution, ordered daily bars/actions, pagination overlap, cancellation, rate limiting, retries, and secret-safe failures in `server/internal/marketdata/provider_test.go`; run and record the expected red behavior
- [ ] T021 [P] [US2] Add EODHD request/mapping tests using only a local HTTP test server for Nordic symbol mapping, decimals, exchange-local dates, adjusted close, actions, timeouts, and sanitized errors in `server/internal/marketdata/eodhd/client_test.go`; run and record the expected red behavior
- [ ] T022 [P] [US2] Add valid-bar/session/action validation unit tests in `server/internal/marketdata/validate_test.go`; run and record the expected red behavior
...
- [ ] T026 [US2] Add ordered market-data schema for exchange sessions, runs/items, current daily bars, immutable revisions, corporate actions, and quality findings in `server/internal/db/migrations/0004_market_data.sql`
- [ ] T027 [US2] Add application-owned official Nordic exchange sessions for the historical window and next complete year as a forward migration in `server/internal/db/migrations/0005_nordic_calendars.sql`
- [ ] T028 [US2] Define the replaceable provider contract and normalized provider records in `server/internal/marketdata/provider.go`
- [ ] T029 [US2] Implement the standard-library EODHD HTTP adapter with bounded retry/backoff, cancellation, pagination normalization, and secret sanitization in `server/internal/marketdata/eodhd/client.go`
- [ ] T030 [US2] Implement exact OHLCV, session, and source-action validation for accepted records in `server/internal/marketdata/validate.go`
...
- [ ] T037 [US3] Add integration tests for succeeded/partial/failed run transitions, atomic item counts, sanitized failures, unresolved findings, last-valid-value retention, and scoped retry lineage in `server/internal/marketdata/health_integration_test.go`; run and record the expected red behavior
- [ ] T038 [P] [US3] Add read-only HTTP contract tests for `/api/v1/market-data/imports`, run detail, and quality-finding filters/error envelopes in `server/internal/api/marketdata_test.go`; run and record the expected red behavior
...
- [ ] T040 [P] [US3] Add frontend service/component tests for recent-run status, completion refresh within 10 seconds, counts, safe errors, retry command copying, warnings/errors with text plus color, loading, and failure states in `src/services/marketData.test.ts` and `src/components/finance/MarketDataStatus.test.ts`; run and record the expected red behavior
- [ ] T041 [US3] Add Playwright failed/partial-import status journeys at 360x800, 768x1024, and 1440x900 plus keyboard/touch and 320x800 overflow checks in `e2e/market-data.spec.ts`; run and record the expected missing-UI red behavior
...
- [ ] T043 [US3] Implement run/item transition accounting, safe error normalization, finding persistence/filtering, and failed-scope reconstruction in `server/internal/marketdata/repository.go` and `server/internal/marketdata/service.go`
- [ ] T044 [US3] Implement `marketdata retry` through the shared service in `server/cmd/market-lens/main.go`
- [ ] T045 [US3] Implement thin import-run/detail and quality-finding handlers in `server/internal/api/marketdata.go`, register them in `server/internal/api/router.go`, and keep generic transport behavior in `server/internal/httpx/`
- [ ] T046 [US3] Implement typed read-only status access and accessible status/quality presentation in `src/types/marketData.ts`, `src/services/marketData.ts`, `src/components/finance/MarketDataStatus.vue`, and `src/components/finance/QualityBadge.vue`
- [ ] T047 [US3] Integrate the responsive market-data status section, bounded polling that exposes final outcomes within 10 seconds, and explicit safe host retry instructions in `src/views/MarketsView.vue` and `src/router/index.ts`
...
- [ ] T061 [P] Add a provider-neutral fixture import acceptance test covering 100-instrument scale, deterministic repeat, correction, action, and quality summaries in `server/internal/marketdata/acceptance_integration_test.go`
